...
const getAll = async () => {
  return await prisma.tipo_orden.findMany({
    select: {
      id: true,
      titulo: true,
      createAt: true,
...
const getById = async (id = 0) => {
  ...
  const tipoOrden = await prisma.tipo_orden.findUnique({
    where: { id: Number(id) },
    select: {
      id: true,
      titulo: true,
  ...
  if (!tipoOrden) throw new BadRequestError("Tipo Orden no Encontrado", 404);
...
const create = async (body = null) => {
  ...
  return await prisma.tipo_orden.create({
    data: { ...body },
    select: {
      id: true,
      titulo: true,
...
const update = async (id = 0, body = null) => {
  ...
  const updateTipoOrden = prisma.tipo_orden.update({
    where: { id: Number(id) },
    data: { ...body },
  });

  const tipoOrden = prisma.tipo_orden.findUnique({
    where: { id: Number(id) },
    select: {
      id: true,
      titulo: true,
...
const _delete = async (id = 0) => {
  ...
  return await prisma.tipo_orden.delete({
    where: { id: Number(id) },
    select: {
      id: true,
      titulo: true,
